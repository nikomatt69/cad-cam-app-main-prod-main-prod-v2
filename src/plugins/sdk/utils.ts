...
export async function isPluginProject(directory: string): Promise<boolean> {
  try {
    // Check if manifest.json exists (new format)
    await fs.access(path.join(directory, 'manifest.json'));
    return true;
  } catch (error) {
    try {
      // Backward compatibility: Check if plugin.json exists (old format)
      await fs.access(path.join(directory, 'plugin.json'));
      console.warn(chalk.yellow('⚠️ Legacy plugin.json format detected. Consider migrating to manifest.json.'));
      return true;
    } catch (error) {
      return false;
    }
