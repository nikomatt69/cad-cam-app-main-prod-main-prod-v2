...
/**
 * Copy the plugin manifest to the dist directory
 */
async function copyPluginManifest(pluginDir: string): Promise<void> {
  try {
    // First try to check if manifest.json exists (new format)
    try {
      await fs.access(path.join(pluginDir, 'manifest.json'));
      
      // Copy manifest.json to dist directory
      await fs.mkdir(path.join(pluginDir, 'dist'), { recursive: true });
      await fs.copyFile(
        path.join(pluginDir, 'manifest.json'),
        path.join(pluginDir, 'dist', 'manifest.json')
      );
      
      console.log(chalk.blue('📄 Copied manifest.json to dist directory'));
      return;
    } catch (error) {
      // manifest.json doesn't exist, try plugin.json (old format)
    }
    
    // Check if plugin.json exists (old format)
    ...
    // Also copy to manifest.json in dist for forward compatibility
    await fs.copyFile(
      path.join(pluginDir, 'plugin.json'),
      path.join(pluginDir, 'dist', 'manifest.json')
    );
    ...
    console.log(chalk.yellow('⚠️ Also copied to manifest.json for compatibility. Consider migrating your plugin.'));
  } catch (error) {
    console.warn(chalk.yellow('⚠️ No manifest.json or plugin.json found.'));
  }
}
