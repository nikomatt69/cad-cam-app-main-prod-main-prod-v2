...
export interface TemplateConfig {
  ...
  /** Template type */
  template: 'basic' | 'sidebar' | 'toolbar' | 'full' | 'utility' | 'custom';
  
  /** Selected features (for custom template) */
  features?: string[];
...
/**
 * Generate a manifest.json file for the plugin
 */
async function generateManifest(config: TemplateConfig): Promise<void> {
  ...
  switch (config.template) {
    ...
    case 'custom':
      // Add custom contributions based on features
      if (config.template.includes('sidebar')) {
        permissions.push('ui:sidebar');
        contributes.sidebar = {
          title: config.name,
          icon: "assets/icon.svg",
      ...
      if (config.template.includes('toolbar')) {
        permissions.push('ui:toolbar');
...
import type { CADCAM } from '@cadcamfun/cadcamfun-plugins-sdk';
...
export async function activate(api: CADCAM.PluginAPI): Promise<void> {
  ...
  api.model.onSelectionChanged((selection: CADCAM.EntityId[]) => {
    console.log('Selection changed:', selection);
  });` : ''}
  ${['toolbar', 'full'].includes(config.template) || (config.template === 'custom' && config.features?.includes('commands')) ? `
