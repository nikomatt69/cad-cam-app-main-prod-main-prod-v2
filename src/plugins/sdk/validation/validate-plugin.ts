...
async function validatePluginDirectory(directory: string): Promise<ValidationResult> {
  ...
  // Check if manifest.json or plugin.json exists
  const manifestPath = path.join(directory, 'manifest.json');
  const pluginJsonPath = path.join(directory, 'plugin.json');
  let manifest;
  ...
  try {
    ...
    try {
      const manifestContent = await fs.readFile(pluginJsonPath, 'utf-8');
      manifest = JSON.parse(manifestContent);
      usingLegacyFormat = true;
      console.warn(chalk.yellow('⚠️ Using legacy plugin.json format. Consider migrating to manifest.json with "cadcam-plugin-cli migrate".'));
    } catch (innerError) {
      errors.push(`Failed to read manifest.json or plugin.json: ${error}`);
  ...
  if (manifest.main) {
    ...
    errors.push('No main file specified in manifest');
  }
  ...
  // Add warning for legacy format
  if (usingLegacyFormat) {
    errors.push('Using legacy plugin.json format. Consider migrating to manifest.json with "cadcam-plugin-cli migrate".');
  }
