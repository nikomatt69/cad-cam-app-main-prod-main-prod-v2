...
const OpenAISetupPanel: React.FC<OpenAISetupPanelProps> = ({ className = '' }) => {
  ...
  const testConnection = async () => {
    ...
    try {
      // Set keys temporarily for testing - NOTE: OpenAIService likely needs configuration update or direct API key passing if not already handled
      // We need to update OpenAIService or how its config is managed to use the apiKey entered here.
      // For now, assuming the proxy uses the environment variable or a configured key.
      
      // Attempt a simple API call using sendMessage
      const testMessages: AIMessage[] = [
        {
          id: 'test-' + Date.now(),
          role: 'user',
          content: 'This is a test message. Please respond with "OK" if you can read this.',
          timestamp: Date.now()
        }
      ];
      const response = await openAIService.sendMessage(
        testMessages, 
        'Connection Test Context', // Minimal context
        [] // No actions needed for test
      );
      
      // Check the content of the response
      if (response && response.content && response.content.trim().toUpperCase() === 'OK') {
        setTestResult({
          success: true,
        ...
        setTestResult({
          success: false,
          message: `Errore: Risposta non valida da OpenAI. Risposta ricevuta: ${response?.content || 'N/A'}`
        });
      }
    } catch (error) {
      setTestResult({
