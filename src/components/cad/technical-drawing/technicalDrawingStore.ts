...
export const useTechnicalDrawingStore = create<TechnicalDrawingState>((set, get) => ({
  ...
  setEntities: (entities: Record<string, DrawingEntity>) => {
    set(state => {
      // Check if the new entities are different from the current ones
      // This prevents unnecessary updates and infinite loops
      if (JSON.stringify(entities) === JSON.stringify(state.entities)) {
        return state; // Return the state unchanged if entities are the same
      }
  ...
  setDimensions: (dimensions: Record<string, Dimension>) => {
    set(state => {
      // Check if the new dimensions are different from the current ones
      // This prevents unnecessary updates and infinite loops
      if (JSON.stringify(dimensions) === JSON.stringify(state.dimensions)) {
        return state; // Return the state unchanged if dimensions are the same
      }
  ...
  setAnnotations: (annotations: Record<string, Annotation>) => {
    set(state => {
      // Check if the new annotations are different from the current ones
      // This prevents unnecessary updates and infinite loops
      if (JSON.stringify(annotations) === JSON.stringify(state.annotations)) {
        return state; // Return the state unchanged if annotations are the same
      }
  ...
  setDrawingLayers: (layers: DrawingLayer[]) => {
    set(state => {
      // Check if the new layers are different from the current ones
      // This prevents unnecessary updates and infinite loops
      if (JSON.stringify(layers) === JSON.stringify(state.drawingLayers)) {
        return state; // Return the state unchanged if layers are the same
      }
  ...
  // Aliases for undoCommand and redoCommand for better API ergonomics
